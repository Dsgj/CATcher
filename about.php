<?php
include 'header.php';
?>
<!-- Page Content -->
<div class="container">
    <!-- Page Heading/Breadcrumbs -->
    <div class="row">
        <div class="col-lg-12">
            <h1 class="page-header">About
                <small>This is CATcher</small>
            </h1>
            <ol class="breadcrumb">
                <li><a href="index.php">Home</a>
                </li>
                <li class="active">About</li>
            </ol>
        </div>
    </div>
    <!-- /.row -->

    <!-- Intro Content -->
    <div class="row">
        <div class="col-lg-12">
            <div class="embed-responsive embed-responsive-16by9">
                <iframe
                    src="https://docs.google.com/presentation/d/11TwO2rzVcU0Cf8nlgVvWjjC5q4YiJZMdHpiyYSP2FSc/embed?start=false&loop=false&delayms=3000"
                    frameborder="0" allowfullscreen="true" mozallowfullscreen="true"
                    webkitallowfullscreen="true"></iframe>
            </div>
        </div>

    </div>
    <!-- /.row -->

    <!-- Team Members -->
    <div class="row">
        <div class="col-lg-12">
            <h2 class="page-header">Our Team</h2>
        </div>
        <div class="col-md-4 col-sm-6 col-xs-12 text-center">
            <div class="thumbnail">
                <img class="img-responsive img-rounded" src="http://oi67.tinypic.com/344umfs.jpg" alt="">

                <div class="caption">
                    <h3>Jesper Gladh<br>
                        <small>Developer</small>
                    </h3>
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Iste saepe et quisquam nesciunt
                        maxime.</p>
                    <ul class="list-inline">
                        <li><a href="https://www.facebook.com/jesper.gladh"><i
                                    class="fa fa-2x fa-facebook-square"></i></a>
                        </li>
                        <li><a href="#"><i class="fa fa-2x fa-linkedin-square"></i></a>
                        </li>
                        <li><a href="https://www.instagram.com/jegladh/"><i class="fa fa-2x fa-instagram"></i></a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="col-md-4 col-sm-6 col-xs-12 text-center">
            <div class="thumbnail">
                <img class="img-responsive img-rounded" src="http://oi64.tinypic.com/wjxvub.jpg" alt="">

                <div class="caption">
                    <h3>Leo Ozolins-Carlson<br>
                        <small>Developer</small>
                    </h3>
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Iste saepe et quisquam nesciunt
                        maxime.</p>
                    <ul class="list-inline">
                        <li><a href="https://www.facebook.com/ozolinscarlson"><i
                                    class="fa fa-2x fa-facebook-square"></i></a>
                        </li>
                        <li><a href="https://se.linkedin.com/in/leoozolinscarlson"><i
                                    class="fa fa-2x fa-linkedin-square"></i></a>
                        </li>
                        <li><a href="https://www.instagram.com/batmanhideout/"><i class="fa fa-2x fa-instagram"></i></a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
        <div class="col-md-4 col-sm-6 col-xs-12 text-center">
            <div class="thumbnail">
                <img class="img-responsive img-rounded"
                     src="https://pbs.twimg.com/profile_images/2171451413/ace-ventura-pet-detective-03-800-75.jpg"
                     alt="">

                <div class="caption">
                    <h3>Ace Ventura, Sr.<br>
                        <small>Pet Detective</small>
                    </h3>
                    <p>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Iste saepe et quisquam nesciunt
                        maxime.</p>
                    <ul class="list-inline">
                        <li><a href="#"><i
                                    class="fa fa-2x fa-facebook-square"></i></a>
                        </li>
                        <li><a href="#"><i
                                    class="fa fa-2x fa-linkedin-square"></i></a>
                        </li>
                        <li><a href="#"><i class="fa fa-2x fa-instagram"></i></a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>
    </div>
    <!-- /.row -->
    <hr>


    <?php
    include 'footer.php';
    ?>
